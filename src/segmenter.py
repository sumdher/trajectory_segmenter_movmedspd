...
class MovMedSpeedSegmenter:
    ...
    def scipy_peaks_plot(self, real=False, tol=1.95):
        ...
        def _on_club_segments_click(_):
            ...
            with output:
                ...
                self.merge_segments_df(duration)
                _label_do()
                median_seg = True
                _update_plot()
    ...
    def segment_graph(
        self, mode=None, window_size=None, steep=0.2) -> pd.DataFrame:
        """
        ...
        def _find_steep_decreases(threshold_ratio=0.2):
            """
            1. Finds the height threshold parameter for Scipy's `find_peaks`.            
            2. By fitting a horizontal line from bottom, looking at the 
                intersections with the graph.
            ...
            for person_id, group in self.traj_df.groupby('person_id'):
                ...
                if person_id not in [h[0] for h in heights]:
                    heights.append(
                        [
                            person_id, 
                            self.traj_df[self.traj_df['person_id'] == person_id]
        ...
        _find_steep_decreases(steep)  # populates "height_threshold_df"
